...
def extract_normal_patch(fullmammo_img, mask_img, normal_patch_per_img, one_more):
    ...
    for i in range(normal_patch_per_img + one_more):#TODO: Check
...
if __name__ == "__main__":
    ...
    #Note: normal patches number must be equal with number of mass patches (num_of_mass_patch)
    num_of_mass_patch = 31417 - 30633
    num_of_images = 1270
    normal_patch_per_img = num_of_mass_patch // num_of_images
    remainder = np.mod(num_of_mass_patch, num_of_images)
    random.shuffle(roi_folders)
    i = 0
    for x in roi_folders:
        ...
        [mask_img, fullmammo_img, fullmammo_dcm_file, num_of_images] = temp

        if(i < remainder):
            extract_normal_patch(fullmammo_img, mask_img, normal_patch_per_img, True)
        else:
            extract_normal_patch(fullmammo_img, mask_img, normal_patch_per_img, False) 
        i = i + 1